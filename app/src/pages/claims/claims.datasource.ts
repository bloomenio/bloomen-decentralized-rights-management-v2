...
export class ClaimsDataSource implements DataSource<UserModel> {
    ...
    public claimsHood$ = new BehaviorSubject<UserModel[]>([]);
    private loadingSubject = new BehaviorSubject<boolean>(false);
    private loadingSubjectHood = new BehaviorSubject<boolean>(false);
    public cmo: string;
    public user: UserModel;
    public loading$ = this.loadingSubject.asObservable();
    public members: MemberModel[];
    public claims: any;
    public claimsHood: any;
    public prevClaims: any;
    private pageSizeFromSol: number;
    ...
    public loadClaims(filter = '', sortDirection = 'asc', pageIndex = 0,
                      pageSize = this.pageSizeFromSol) {
    ...
    public loadClaimsUnderTheHood(filter = '', sortDirection = 'asc', pageIndex = 0,
                                  pageSize = this.pageSizeFromSol) {
        this.loadingSubjectHood.next(true);
        this.claimsContract.getClaimsByMemId(pageIndex).then((claims) => {
            claims = claims.sort((a, b) => (a[2][0][1] === b[2][0][1]
                ? (a[6] - b[6]) : (a[2][0][1]  < b[2][0][1] ? -1 : 1)));
            this.claimsHood = claims;
            let previous: any;
            let temp: any;
            if (claims.length) {
                temp = claims[0];
            ...
            this.claimsHood$.next(claims);
            this.loadingSubjectHood.next(false);
        });
    }
